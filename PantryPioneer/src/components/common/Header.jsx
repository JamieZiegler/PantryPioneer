...
export default function Header() {
    const styles = {
        navLink:
            "group relative flex h-full items-center gap-3 px-3 min-[981px]:px-2 font-body font-light text-[0.95rem] !text-white transition-all duration-200 cursor-pointer after:content-[''] after:absolute after:bottom-0 after:inset-x-0 after:h-[3px] after:bg-accent after:z-10 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300",
        icon: "h-7 w-7 min-[981px]:h-6 min-[981px]:w-6 transition-transform duration-200",
        logoLink:
            "relative flex h-full items-center px-3 cursor-pointer after:content-[''] after:absolute after:bottom-0 after:inset-x-0 after:h-[3px] after:bg-accent after:z-10 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300",
        accountAction:
            "group relative flex h-full items-center gap-2.5 px-3 min-[981px]:px-2 font-body font-light text-[0.95rem] !text-white whitespace-nowrap transition-all duration-200 cursor-pointer after:content-[''] after:absolute after:bottom-0 after:inset-x-0 after:h-[3px] after:bg-accent after:z-10 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300",
        accountIcon: "h-6 w-6 min-[981px]:h-5 min-[981px]:w-5",
        toggleButton:
            "group relative flex h-full items-center px-4 min-[981px]:px-3 text-white/50 hover:text-white duration-200 transition-colors cursor-pointer",
        toggleIcon:
            "h-7 w-8 min-[981px]:h-6 min-[981px]:w-7 transition-transform duration-200 ease-out",
        slideContainer:
            "overflow-hidden transition-[max-width,opacity,transform] duration-400 ease-out",
        slideFavorites:
            "overflow-hidden transition-[max-width,opacity,transform] duration-400 ease-out",
        slideLogout:
                    ...
                    <li className="flex h-full items-stretch justify-end">
                        {!user ? (
                            ...
                            <div className="flex h-full items-stretch justify-end">
                                ...
                                <div
                                    className={`${styles.slideContainer} ${slideoutOpen ? "max-w-152 translate-x-0 opacity-100 delay-75" : "max-w-0 translate-x-2 opacity-0 delay-0"}`}
                                >
                                    <div className="flex h-full items-stretch">
                                        <div
                                            className={`${styles.slideFavorites} ${slideoutOpen ? "max-w-48 translate-x-0 opacity-100 delay-75" : "max-w-0 translate-x-2 opacity-0 delay-0"}`}
                                        >
                                            <Link
                                                to="/favorites"
                                        ...
                                        <div
                                            className={`${styles.slideLogout} ${slideoutOpen ? "max-w-44 translate-x-0 opacity-100 delay-200" : "max-w-0 translate-x-2 opacity-0 delay-0"}`}
                                        >
                                            <button
                                ...
                                >
                                    {slideoutOpen ? (
                                        <ChevronsRight
                                            className={`${styles.toggleIcon} group-hover:translate-x-1`}
                                            strokeWidth={1.5}
                                        />
                                    ) : (
                                        <ChevronsLeft
